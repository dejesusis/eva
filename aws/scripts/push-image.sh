...
if [ $# -eq 0 ]; then
  echo "No arguments provided."
    ...
    if [ "$1" == "base" ]; then
        docker build --provenance=false --build-arg BASE_IMAGE=$AWS_ACCOUNT_ID.dkr.ecr.us-east-1.amazonaws.com/shiny:$SHINY_VERSION -t eva/base -f aws/docker/Dockerfile.Base .
